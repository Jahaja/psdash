...
class TestLogs(unittest2.TestCase):
    ...
    def test_searching(self):
        ...
        positions = [
            log.search(self.NEEDLE)[0] for _ in range(len(self.POSITIONS))
        ]
    ...
    def test_add_non_existing(self):
        self.assertRaises(
            LogError, self.logs.add_available, "/var/log/w0ntre4lly3xist.log"
        )
...
class TestFileSearcher(unittest2.TestCase):
    ...
    def test_unicode(self):
        encoding = "utf-8"
        needle = "Åter till testerna, så låt oss nu testa lite".encode(
            encoding
        )
        buf = StringIO()
        data = (
            "Det är nog bra att ha några konstiga bokstäver här med.\n" * 10000
        ).encode(encoding)
        buf.write(data)
